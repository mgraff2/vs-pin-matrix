...
# commit. Roughly 8 versions x 7 combos x ~30s boot, so budget half an hour; it is not a
# per-edit test (that's compat-test.ps1 against the installed server).
#
# Pin Matrix is client-side only, so what this sweeps is narrower than for a universal mod:
# per game version it proves the zip unpacks, modinfo/dependency declarations are accepted,
# PinMatrix.dll still LOADS against that version's assemblies, and the Client-side gate holds.
# It cannot see the GUI. What it is really guarding is the `"game": "1.22.0"` floor — the DLL
# is built against one game version's references and claimed to work on all of them.
#
...
#
# Server packages (~300MB extracted each) are downloaded from the official CDN and cached,
